...
新一代 **Coding Agent Harness（编码代理基座）**，定位「抬升技能上限（raise the skill ceiling）」。面向**多会话工作流**、无限可定制与高性能，跨 Linux / macOS / Windows。内置记忆系统（memory），强调 RAM 占用与启动速度优化，适合规模化多会话场景。

一句话：把编码 Agent 做成「可插拔、可并行、低开销」的 harness，而非单一闭环助手。

## 技术栈

- **语言/运行时**：Rust（编译型，跨平台二进制，TUI 界面）
- **形态**：终端编码 Agent / Harness，支持多会话并行
- **模型接入**：Claude / OpenAI 等（topics 含 claude, openai, llm, mcp）
- **集成**：MCP（model-context-protocol）
- **Topics**：ai-agent, ai-coding-agent, coding-agent, cli, mcp, rust, terminal, tui

## 外部链接

- GitHub：https://github.com/1jehuang/jcode
- 作者：https://github.com/1jehuang
- 官网：https://jcode.sh
- 安装：macOS & Linux `curl -fsSL https://jcode.sh/install | bash`；Windows `irm https://jcode.sh/install.ps1 | iex`
