---
aliases: [skills, mattpocock/skills]
tags: [AI, Trending, Shell, Claude-Code, Skills, Vibe-Coding, Engineering]
stars: 234648
today_growth: 2447
created_at: 2026-02-03
status: 11度上榜
date_accessed: 2026-08-24
---

# skills

**项目地址**：https://github.com/mattpocock/skills
**作者**：mattpocock（Total TypeScript 作者）
**⭐ 总 Star**：209,330（209.3k）  <!-- 2026-08-08 18:05 API 实时值；当日 08:25 为 208,799，08-07 为 207,963 -->
**📈 今日新增**：🔺2,152 stars（榜面值；**完整 24h 实测 +1,367（207,963→209,330），榜面高估 57%**）
**🍴 Fork**：18,022
**💻 主要语言**：Shell
**🗓 开源时间**：2026-02-03
**🔄 最近推送**：2026-08-07 15:50
**🌐 官网**：https://aihero.dev/skills
**📜 许可证**：MIT

## 项目定位

Matt Pocock（Total TypeScript 作者）日常在真实工程中使用的 **AI Agent 技能集**，定位「为真实工程师服务，而非 vibe coding」。强调技能**小而可组合、跨模型通用**，基于数十年工程经验沉淀；与 GSD / BMAD / Spec-Kit 等「接管流程」方案不同，这些技能保留开发者控制权，便于 hack 与改造。安装：`npx skills@latest add mattpocock/skills`。

## 技术栈

- 形态：纯 Markdown 技能文件（SKILL.md），通过 skills.sh 分发
- 运行：Shell 安装脚本；兼容任意模型与 Agent（Claude Code 等）
- 设计哲学：可组合、易适配、工程纪律优先
- 含 grill-me / tdd / to-spec 等 39 项技能；支持 GitHub / Linear / 本地文件 Issue 跟踪

## 外部链接

- GitHub：https://github.com/mattpocock/skills
- 作者：https://github.com/mattpocock

## 相关日期

- [[Vibe-Coding-2026-07-11|2026-07-11 日报]]
- [[Vibe-Coding-2026-07-15|2026-07-15 日报]]
- [[Vibe-Coding-2026-07-16|2026-07-16 日报]]
- [[Vibe-Coding-2026-07-17|2026-07-17 日报]]
- [[AI-Weekly-2026-07-19|2026-07-19 周报]]

- [[AI-Weekly-2026-07-26|2026-07-26 周报]]
- [[Vibe-Coding-2026-08-07|2026-08-07 日报]]（#2，⭐208.0k / 🔺1,873）
- [[Vibe-Coding-2026-08-08|2026-08-08 日报]]（#2，⭐208.8k / 🔺2,152）

## 备注

- 2026-07-26 周报（周末全赛道 Top 10，#3）：本周 +11,790，总 Star 188.2k（188,239）。

- 由 GitHub Trending 周榜自动归档（周末全赛道 Top 10，#2）。本周 +11,325，总 Star 17.7 万，稳居「技能即工程纪律」赛道头部。

- **格式修复（2026-08-08）**：本页 YAML frontmatter 自建档起缺失闭合 `---`，Obsidian 无法解析属性，本次修复；同时把陈旧的 `weekly_growth` 字段改为 `today_growth`，与其余项目页对齐。

- **成长曲线**：2026-07-26 ⭐188,239 → 2026-08-07 ⭐207,963 → 2026-08-08 ⭐208,799。12 天 +19.7k 跨过 20 万门槛，此后维持日均 ~1.5k 的稳定增速。

- 2026-08-08 日报（Vibe Coding #2）：与 [[addyosmani_agent-skills|addyosmani/agent-skills]]（83.9k / 🔺1,131）、[[obra_superpowers|obra/superpowers]]（268.7k / 🔺782）**同日占据 Top 5 三席，合计 56.1 万星、当日合计 🔺4,065**。三者在「自由度」轴上是三个刻度：本项目给零件不管怎么装，addyosmani 给零件外加质检表，superpowers 直接给整条产线。

## 反向链接
- [[Daily/Vibe-Coding-2026-08-20.md|2026-08-20 收录]]
- [[Daily/Vibe-Coding-2026-08-22.md|2026-08-22 收录]]
- [[Daily/AI-Weekly-2026-08-23.md|2026-08-23 收录]]
- [[Daily/Vibe-Coding-2026-08-24.md|2026-08-24 收录]]
