---
aliases: [ai-agent-book]
tags: [AI, Trending, Python, agent, agent-memory, ai-agent, book, coding-agent, context-engineering, large-language-models, llm]
stars: 39099
weekly_growth: 16579
created_at: 2025-09-09
status: 4度上榜
date_accessed: 2026-08-19
---

# ai-agent-book

**项目地址**：https://github.com/bojieli/ai-agent-book
**作者**：bojieli
**⭐ 总 Star**：19,767（19.8k）
**📈 本周新增**：16,579 stars
**💻 主要语言**：Python
**🗓 开源时间**：2025-09-09

## 项目定位

李博杰（bojieli）所著《深入理解 AI Agent：设计原理与工程实践》的官方开源仓库。包含全书正文（LaTeX/Markdown）、编译版 PDF 与按章配套代码，系统讲解 Agent 记忆、上下文工程、编码 Agent 等原理与落地实践，是中文世界少见的 AI Agent 工程化系统教材。

## 技术栈

- 形态：开源教材（书）+ 按章配套代码
- 语言：Python（示例/工具链）
- 主题：agent-memory / context-engineering / coding-agent / book
- 交付：正文 + 编译版 PDF + 逐章代码

## 外部链接

- GitHub：https://github.com/bojieli/ai-agent-book
- 作者：https://github.com/bojieli

## 相关日期

- [[AI-Weekly-2026-07-26|2026-07-26 周报]]

## 备注

- 由 GitHub Trending 周榜自动归档（周末全赛道 Top 10，#1）。本周 +16,579，总 Star 19.8k（19,767）。

## 反向链接
- [[Daily/Vibe-Coding-2026-08-19.md|2026-08-19 收录]]
