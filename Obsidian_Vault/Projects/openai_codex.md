---
aliases: [codex]
tags: [AI, Trending, Rust, Coding-Agent, OpenAI, Terminal, Full-Track]
stars: 118558
weekly_growth: 2250
created_at: 2025-04-13
status: 5度上榜
date_accessed: 2026-08-26
---

# codex

**项目地址**：https://github.com/openai/codex
**作者**：openai
**⭐ 总 Star**：99,404（99.4k）
**📈 本周新增**：2,250 stars
**💻 主要语言**：Rust
**🗓 开源时间**：2025-04-13
**许可证**：Apache-2.0

## 项目定位

OpenAI 官方**轻量编码 Agent**，运行于终端——把自然语言转成代码改动并自动执行、跑测试、提交。Codex 模型驱动，是官方把「Agentic coding」产品化的标杆，也是「终端编码 Agent」赛道的参照系（9.9 万 Star，本周 +2,250）。

## 技术栈

- 语言：Rust
- 形态：终端编码 Agent（CLI）
- 归属：OpenAI 官方
- 许可证：Apache-2.0

## 外部链接

- GitHub：https://github.com/openai/codex
- 组织：https://github.com/openai

## 相关日期

- [[AI-Weekly-2026-07-19|2026-07-19 周报]]

## 备注

- 由 GitHub Trending 周榜自动归档（周末全赛道 Top 10，#9）。

## 反向链接
- [[Daily/AI-Weekly-2026-08-23.md|2026-08-23 收录]]
- [[Daily/Vibe-Coding-2026-08-24.md|2026-08-24 收录]]
- [[Daily/Vibe-Coding-2026-08-25.md|2026-08-25 收录]]
- [[Daily/Vibe-Coding-2026-08-26.md|2026-08-26 收录]]
