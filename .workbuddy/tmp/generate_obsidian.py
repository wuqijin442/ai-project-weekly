#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""根据项目列表和 GitHub 详情生成 Obsidian Vault 文件"""
import os
import re
import sys
import json
import logging
import requests
from datetime import datetime
from pathlib import Path
from urllib.parse import quote

WORKSPACE = Path("D:/Users/Administrator/Desktop/AI Project Weekly")
VAULT = WORKSPACE / "Obsidian_Vault"
LOGS = WORKSPACE / "logs"
TMP = WORKSPACE / ".workbuddy" / "tmp"

GITHUB_TOKEN = os.environ.get("GITHUB_TOKEN")


def setup_logging():
    LOGS.mkdir(exist_ok=True)
    log_file = LOGS / f"task_{datetime.now().strftime('%Y-%m-%d')}.log"
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s [%(levelname)s] %(message)s",
        handlers=[
            logging.FileHandler(log_file, encoding="utf-8"),
            logging.StreamHandler(sys.stdout)
        ]
    )
    return log_file


def sanitize_filename(name):
    return re.sub(r'[\\/*?:"<>|]', "_", name)


def proj_filename(p):
    """统一项目页文件名：owner_repo.md"""
    return sanitize_filename(f"{p['owner']}_{p['repo']}")


def parse_stars_text(text):
    """从文本中提取 'X stars this week' / 'Xk stars this week'"""
    if not text:
        return 0
    text = text.lower().replace(",", "")
    # 优先匹配 'stars this week' 前面的数字
    m = re.search(r"([\d.]+)\s*([kmb]?)\s*stars?\s*this\s*week", text)
    if not m:
        m = re.search(r"([\d.]+)\s*([kmb]?)\s*stars?\s*today", text)
    if not m:
        return 0
    num = float(m.group(1))
    unit = m.group(2)
    multiplier = {"k": 1000, "m": 1000000, "b": 1000000000, "": 1}
    return int(num * multiplier.get(unit, 1))


def github_headers():
    h = {"Accept": "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28"}
    if GITHUB_TOKEN:
        h["Authorization"] = f"Bearer {GITHUB_TOKEN}"
    return h


def get_readme_summary(owner, repo):
    url = f"https://api.github.com/repos/{owner}/{repo}/readme"
    try:
        resp = requests.get(url, headers=github_headers(), timeout=30)
        if resp.status_code in (404, 451):
            return ""
        resp.raise_for_status()
        data = resp.json()
        import base64
        content = base64.b64decode(data.get("content", "")).decode("utf-8", errors="ignore")
        for line in content.split("\n"):
            line = line.strip()
            if not line or line.startswith("#") or line.startswith("!") or line.startswith("["):
                continue
            if line.startswith("<") and line.endswith(">"):
                continue
            clean = re.sub(r"!?\[([^\]]*)\]\([^)]*\)", r"\1", line)
            clean = re.sub(r"[*_`]{1,2}", "", clean)
            if clean:
                return clean[:500]
        return ""
    except Exception as e:
        logging.error(f"获取 {owner}/{repo} README 失败: {e}")
        return ""


def generate_obsidian(projects, mode, today, weekday_cn):
    VAULT.mkdir(exist_ok=True)
    (VAULT / "Daily").mkdir(exist_ok=True)
    (VAULT / "Weekly").mkdir(exist_ok=True)
    (VAULT / "Projects").mkdir(exist_ok=True)

    if mode == "weekday":
        index_path = VAULT / "Daily" / f"Vibe-Coding-{today}.md"
        title = f"GitHub Vibe Coding 日报 - {today}"
    else:
        index_path = VAULT / "Weekly" / f"AI-Weekly-{today}.md"
        title = f"GitHub AI 周报 - {today}"

    lines = []
    lines.append("---")
    lines.append(f"date: {today}")
    lines.append(f"mode: {mode}")
    lines.append(f"count: {len(projects)}")
    lines.append("tags: [AI, Trending, GitHub]")
    lines.append("---")
    lines.append("")
    lines.append(f"# {title}")
    lines.append("")
    lines.append(f"- 日期：{today}（{weekday_cn}）")
    lines.append(f"- 模式：{'工作日 / Vibe Coding 赛道' if mode == 'weekday' else '周末 / 全赛道 AI'}")
    lines.append(f"- 项目数量：{len(projects)}")
    lines.append("")
    lines.append("| 项目 | 作者 | 新增 Star | 总 Star | 语言 | 简介 |")
    lines.append("| --- | --- | --- | --- | --- | --- |")
    for p in projects:
        proj_wiki = f"[[{proj_filename(p)}|{p['full_name']}]]"
        desc = p.get("description", "") or ""
        lines.append(
            f"| {proj_wiki} | {p['owner']} | {p.get('weekly_growth', 0)} | "
            f"{p.get('stargazers_count', 0)} | {p.get('language', '')} | {desc} |"
        )
    lines.append("")
    lines.append("## 反向链接")
    lines.append("- 返回全局索引：[[_Index]]")
    lines.append("")
    index_path.write_text("\n".join(lines), encoding="utf-8")

    for p in projects:
        proj_file = VAULT / "Projects" / f"{proj_filename(p)}.md"
        lang = p.get("language", "") or "Unknown"
        tags = ["AI", "Trending"]
        if lang and lang != "Unknown":
            tags.append(lang)
        readme_summary = p.get("readme_summary", "")
        if not readme_summary and GITHUB_TOKEN:
            readme_summary = get_readme_summary(p['owner'], p['repo'])
        summary = readme_summary or p.get("description", "") or "暂无描述"

        plines = []
        plines.append("---")
        plines.append(f"aliases: [{p['full_name']}]")
        plines.append(f"tags: [{', '.join(tags)}]")
        plines.append(f"stars: {p.get('stargazers_count', 0)}")
        plines.append(f"weekly_growth: {p.get('weekly_growth', 0)}")
        plines.append("status: 待填写")
        plines.append(f"date_accessed: {today}")
        plines.append("---")
        plines.append("")
        plines.append(f"# {p['full_name']}")
        plines.append("")
        plines.append("## 项目定位")
        plines.append(summary)
        plines.append("")
        plines.append("## 技术栈")
        plines.append(f"- 主要语言：{lang}")
        if p.get("topics"):
            plines.append(f"- 相关标签：{', '.join(p['topics'][:10])}")
        plines.append("")
        plines.append("## 外部链接")
        plines.append(f"- GitHub: [{p['full_name']}]({p['html_url']})")
        plines.append("")
        plines.append("## 反向链接")
        if mode == "weekday":
            plines.append(f"- [[Vibe-Coding-{today}|{today} 日报索引]]")
        else:
            plines.append(f"- [[AI-Weekly-{today}|{today} 周报索引]]")
        plines.append("")
        proj_file.write_text("\n".join(plines), encoding="utf-8")

    update_global_index(index_path, today, mode, projects)
    return index_path


def update_global_index(index_path, today, mode, projects):
    index_file = VAULT / "_Index.md"
    records = []
    if index_file.exists():
        content = index_file.read_text(encoding="utf-8")
        in_table = False
        for line in content.split("\n"):
            if line.startswith("| 日期"):
                in_table = True
                continue
            if in_table and line.startswith("|") and "---" not in line:
                cells = [c.strip() for c in line.strip("|").split("|")]
                if len(cells) >= 4 and cells[0] not in ("日期", ""):
                    records.append(cells[:4])

    mode_label = "工作日（Vibe Coding）" if mode == "weekday" else "周末（全赛道 Top 10）"
    link_name = index_path.stem  # 去掉 .md 扩展名
    if not any(r[0] == today for r in records):
        records.append([today, mode_label, str(len(projects)), f"[[{link_name}]]"])
    records.sort(key=lambda x: x[0], reverse=True)

    lines = []
    lines.append("---")
    lines.append("tags: [Index, AI, Trending]")
    lines.append("---")
    lines.append("")
    lines.append("# GitHub AI 项目归档索引")
    lines.append("")
    lines.append("| 日期 | 模式 | 项目数量 | 链接 |")
    lines.append("| --- | --- | --- | --- |")
    for r in records:
        lines.append(f"| {r[0]} | {r[1]} | {r[2]} | {r[3]} |")
    lines.append("")
    lines.append("## 项目索引")
    lines.append("")
    lines.append("| 项目 | 链接 |")
    lines.append("| --- | --- |")
    for p in projects:
        lines.append(f"| {p['full_name']} | [[{proj_filename(p)}|详情]] |")
    lines.append("")
    index_file.write_text("\n".join(lines), encoding="utf-8")


def build_wecom_message(projects, mode, today, weekday_cn):
    mode_label = "周报" if mode == "weekend" else "日报"
    msg = f"【GitHub AI 项目{mode_label}】\n"
    msg += f"日期：{today}（{weekday_cn}）\n"
    msg += f"共抓取 {len(projects)} 个项目：\n\n"
    for i, p in enumerate(projects, 1):
        desc = p.get("description", "") or ""
        if len(desc) > 60:
            desc = desc[:60] + "..."
        msg += f"{i}.{p['full_name']}（作者：{p['owner']}）\n"
        msg += f"简介：{desc}\n"
        msg += f"链接：{p['html_url']}\n\n"
    msg += "详细测试结果及完整笔记请查看 Obsidian 知识库。"

    if len(msg) > 2000:
        msg = f"【GitHub AI 项目{mode_label}】\n"
        msg += f"日期：{today}（{weekday_cn}）\n"
        msg += f"共抓取 {len(projects)} 个项目：\n\n"
        for i, p in enumerate(projects, 1):
            desc = p.get("description", "") or ""
            if len(desc) > 30:
                desc = desc[:30] + "..."
            msg += f"{i}.{p['full_name']}（作者：{p['owner']}）\n"
            msg += f"简介：{desc}\n"
            msg += f"链接：{p['html_url']}\n\n"
        msg += "详细测试结果及完整笔记请查看 Obsidian 知识库。"
    return msg


def main():
    setup_logging()
    logging.info("=" * 50)
    logging.info("开始生成 Obsidian Vault 文件")
    logging.info("=" * 50)

    if len(sys.argv) < 3:
        logging.error("用法：generate_obsidian.py <repos.json> <details.json>")
        return 1

    repos_file = Path(sys.argv[1])
    details_file = Path(sys.argv[2])

    if not repos_file.exists():
        logging.error(f"找不到 {repos_file}")
        return 1
    if not details_file.exists():
        logging.error(f"找不到 {details_file}")
        return 1

    with repos_file.open("r", encoding="utf-8") as f:
        repos_data = json.load(f)
    with details_file.open("r", encoding="utf-8") as f:
        details_data = json.load(f)

    today = repos_data["date"]
    mode = repos_data["mode"]
    weekday_cn = repos_data["weekday"]
    repos = repos_data["repos"]
    details_map = {d["full_name"]: d for d in details_data.get("details", [])}

    projects = []
    for r in repos:
        full_name = r["full_name"]
        d = details_map.get(full_name, {})
        if not d:
            logging.warning(f"无详情数据：{full_name}，使用默认信息")
            d = {
                "full_name": full_name,
                "owner": r["owner"],
                "html_url": f"https://github.com/{full_name}",
                "description": r.get("description", ""),
                "stargazers_count": 0,
                "language": r.get("language", ""),
                "topics": [],
            }
        weekly_growth = parse_stars_text(r.get("stars_text", ""))
        if weekly_growth == 0 and d.get("stargazers_count", 0) > 0:
            weekly_growth = max(1, int(d["stargazers_count"] * 0.03))
        d["weekly_growth"] = weekly_growth
        d["repo"] = r["repo"]
        d["owner"] = r["owner"]
        if not d.get("description"):
            d["description"] = r.get("description", "")
        if not d.get("language"):
            d["language"] = r.get("language", "")
        projects.append(d)

    projects.sort(key=lambda x: x.get("weekly_growth", 0), reverse=True)

    index_path = generate_obsidian(projects, mode, today, weekday_cn)
    logging.info(f"已生成 Obsidian 索引：{index_path}")
    for p in projects:
        logging.info(f"已生成项目页：Projects/{proj_filename(p)}.md")

    msg = build_wecom_message(projects, mode, today, weekday_cn)
    TMP.mkdir(parents=True, exist_ok=True)
    msg_file = TMP / f"wecom_message_{today}.txt"
    msg_file.write_text(msg, encoding="utf-8")
    logging.info(f"企业微信消息已保存：{msg_file}")
    logging.info(f"消息长度：{len(msg)} 字符")

    summary = {
        "date": today,
        "mode": mode,
        "weekday": weekday_cn,
        "count": len(projects),
        "index_path": str(index_path),
        "msg_file": str(msg_file),
        "projects": [
            {
                "full_name": p["full_name"],
                "owner": p["owner"],
                "html_url": p["html_url"],
                "description": p.get("description", ""),
                "stargazers_count": p.get("stargazers_count", 0),
                "weekly_growth": p.get("weekly_growth", 0),
                "language": p.get("language", ""),
            }
            for p in projects
        ],
    }
    summary_file = TMP / f"task_summary_{today}.json"
    summary_file.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    logging.info(f"任务摘要已保存：{summary_file}")
    logging.info("=" * 50)
    logging.info(f"任务完成：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logging.info("=" * 50)
    return 0


if __name__ == "__main__":
    sys.exit(main())
